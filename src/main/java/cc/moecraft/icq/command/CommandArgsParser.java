...
public class CommandArgsParser {
    ...
    public static CommandArgs parse(CommandManager manager, String fullCommand, boolean isGM)
        ...
        boolean isCommandPrefixesDisabled = manager.isCommandPrefixesDisabled();
        // 获取前缀
        String prefix = isCommandPrefixesDisabled?"":getPrefix(manager.getPrefixes(), fullCommand);
        if(isCommandPrefixesDisabled){
            Set<String> commandNames = manager.getCommandNameSetNoCopy();
            boolean flag = false;
            for (String s : commandNames) {
                if(fullCommand.startsWith(s)){
                    flag = true;
                    break;
                }
            }
            if(!flag) new CommandNotFoundException();
        ...
        // 移除结尾添加的字符
        if(!manager.isCommandPrefixesDisabled()) args.remove(args.size() - 1);
        ...
        // 确认指令存在
        if(!manager.isCommandPrefixesDisabled() && !manager.getCommands().containsKey(command)){
